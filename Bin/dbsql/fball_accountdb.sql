...
DELIMITER ;;
CREATE DEFINER=`ron`@`localhost` PROCEDURE `query_id`(in csid INT, in sdkid INT, IN act VARCHAR(32), OUT xid INTEGER, OUT xname VARCHAR(32) )
BEGIN   
		declare v_id  INT;
...
DELIMITER ;;
CREATE DEFINER=`ron`@`localhost` PROCEDURE `query_update_user_name`(IN uname VARCHAR(32),in csid INT, in sdkid INT,IN ucdkey VARCHAR(32),  OUT xid INT)
BEGIN   
