...
namespace GateServer.Net
{
	public class ClientSession : SrvCliSession
	{
		...
		{
			this._msgHandler.Register( ( int )GCToCS.MsgNum.EMsgToGstoCsfromGcAskLogin, this.OnMsgToGstoCsfromGcAskLogin );
			this._msgHandler.Register( ( int )GCToCS.MsgNum.EMsgToGstoCsfromGcAskReconnectGame, this.OnMsgToGstoCsfromGcAskReconnectGame );
			this._msgHandler.Register( ( int )GCToSS.MsgNum.EMsgToGstoSsfromGcAskPingSs, this.OnMsgToGstoSsfromGcAskPingSs );
			int start = ( int )GCToCS.MsgNum.EMsgToGstoCsfromGcBegin + 1;
			int end = ( int )GCToCS.MsgNum.EMsgToGstoCsfromGcEnd;
			for ( int i = start; i < end; ++i )
				this._msgHandler.Register( i, this.OnTransToCS );
			start = ( int )GCToSS.MsgNum.EMsgToGstoSsfromGcBegin;
			end = ( int )GCToSS.MsgNum.EMsgToGstoSsfromGcEnd;
			for ( int i = start; i < end; ++i )
				this._msgHandler.Register( i, this.OnTransToSS );
		}
